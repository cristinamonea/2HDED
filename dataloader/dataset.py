...
class DatasetFromFolder(data.Dataset):
    def __init__(self, opt, root, phase, data_split, data_augmentation, crop=True, resize=True, data_transform=None, imageSize=[256], outputSize=0, dataset_name='nyu'):
        ...
        self.input_list, self.target_list, self.aif_list = get_paths_list(root, opt.dataset_name, phase, data_split, opt.tasks, opt)
    ...
    def __getitem__(self, index):
        ...
        aif_img = load_img(self.aif_list[index])[0]
        ...
        input_aif_tensor = self.data_augm_obj.apply_image_transform(aif_img)[0]
        
        return input_img_tensor, targets_tensor, input_aif_tensor
