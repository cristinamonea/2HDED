...
##########################
    def _train_batch(self):
        ...
        self.set_current_visuals(aif_gt=aif_data.data)
        ...
        for i_task, task in enumerate(self.opt.tasks):
            ...
            #depth target
            #self.set_current_visuals(target=target.data)
            self.set_current_visuals(depth_gt=target.data,
                                         depth_pred=outG[i_task].data)
...
#                Sloss = ssim(aif_data, aif_pred, data_range=-5)
                ...
                # print ("ssim", Sloss)
                lssim = 0
                if self.opt.aif_reg == 'SSIM':
                    ssim_value = pytorch_ssim.ssim(aif_data, aif_pred).item()
                    lssim = 1 - ssim_value

                
                #######^^^^^ Loss SSIM ^^^^^^^####
                #####VVVVV Calculate AIF LOSS CHARBONNIER VVVV####
                if self.opt.aif_loss == 'Charb':
                    ...
                    losses.append(loss_aif)
                    self.set_current_errors(Charb=loss_aif.item())
                    aif_loss = loss_aif
                ...
                if self.opt.aif_loss == 'L1':
                    l1 = nn.L1Loss()
                    l1_loss=l1(aif_pred,aif_data)
                    losses.append(l1_loss)
                    self.set_current_errors(L1_aif=l1_loss.item())
                    aif_loss = l1_loss
            ...
            # elif task == 'depth':
                ...
                # #dt = depth-output
                # dt = target-output
                # kernely = torch.tensor([[[[1,1,1],[0,0,0],[-1,-1,-1]]]], dtype=torch.float).to(self.device)
                # kernelx = torch.tensor([[[[1,0,-1],[1,0,-1],[1,0,-1]]]], dtype=torch.float).to(self.device)
                ...
                if self.opt.depth_loss == 'L1':
                    l1_depth = nn.L1Loss()
                    l1_loss_depth = l1_depth(outG[0], target)
                    losses.append(l1_loss_depth)
                    self.set_current_errors(L1_depth=l1_loss_depth.item())
                    depth_loss = l1_loss_depth
               
                if self.total_iter % self.opt.print_freq == 0:
                    with torch.no_grad():
                        rmse = sqrt(self.mse_scaled_error(outG[0], target).item()) # mean through the batch
                            
                    self.set_current_errors(RMSE=rmse)
        ...
        # 
        self.loss_error =  self.opt.depth_loss_coef * (depth_loss + self.opt.depth_reg_coef * gradient) + self.opt.aif_loss_coef * (aif_loss + self.opt.aif_reg_coef*lssim)
    ...
    def evaluate(self, data_loader, epoch):
        if self.opt.validate and self.total_iter % self.opt.val_freq == 0:
            ...
            is_best = self.best_val_error > self.val_errors[f'{self.opt.depth_loss}_depth']
            if is_best:     # and not self.opt.not_save_val_model:
                    print("Updating BEST model (epoch {}, iters {})\n".format(epoch, self.total_iter))
                    self.best_val_error = self.val_errors['L1_depth']
    ...
    def get_eval_error(self, data_loader):
        ...
        with torch.no_grad():
            ...
            for _ in pbar_val:
                ...
                input_cpu, target_cpu, aif_cpu = next(data_iter)#.next()
                input_data = input_cpu.to(self.device)
                target_data = target_cpu[0].to(self.device)
                aif_data = aif_cpu.to(self.device)
                ...
                if self.opt.depth_loss == 'L1':
                    l1_depth = nn.L1Loss()
                    depth_err += l1_depth(outG[0], target_data)
                ...
                rmse += sqrt(nn.MSELoss()(target_data, outG[0]))
            ...
            self.val_current_visuals.update([('aif_gt', aif_cpu)])
            self.val_current_visuals.update([('aif_pred', aif_pred)])

            self.val_current_visuals.update([('depth_gt', target_cpu[0])])
            self.val_current_visuals.update([('depth_pred', outG[0])])
            # for i_task, task in enumerate(self.opt.tasks):
            ...
            if self.opt.aif_loss == 'L1':    
                self.val_errors.update([('L1_aif', self.opt.aif_loss_coef * aif_err.cpu().data.numpy()/len(data_loader))])
            if self.opt.aif_loss == 'Charb':
                self.val_errors.update([('Charb_aif', self.opt.aif_loss_coef * aif_err.cpu().data.numpy()/len(data_loader))])

            if self.opt.depth_loss == 'L1':
                self.val_errors.update([('L1_depth', depth_err.cpu().data.numpy()/len(data_loader))])

            self.val_errors.update([('RMSE_depth', rmse/len(data_loader))])
           
            self.val_errors.update([('L2HDED', (self.opt.depth_loss_coef*depth_err + self.opt.aif_loss_coef*aif_err).cpu().data.numpy()/len(data_loader))])
